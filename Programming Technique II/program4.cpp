...
// Exercise 4: Polymorphism and inheritance
...
//! Name 1:JOANNE CHING YIN XUAN,KTDI
//! Name 2:HARINI A/P SANGARAN, KTDI
...
public:
    Mentee(string name="", int level=0);
...
{
    cout << "Mentor's Name: " << getName() << endl;
...
: Mentor(name, cpa), expertise(expertise) {}
...
Mentee::Mentee(string name, int level) : Student(name), level(level), assignedMentor(nullptr){}

void Mentee::assignMentor(Mentor* m) {
	assignedMentor = m;
}

void Mentee::unassignMentor() {
	assignedMentor = nullptr;
}

void Mentee::displayInfo() const {
	cout << "Mentee's Name : " << getName() << endl;
	cout << "Mentee's Level : " << level << endl;
	if (assignedMentor) {
		assignedMentor->print();
	}
	else {
    ...
    while (operation != 9)
    {
        ...
        case 5: // Task 6(f): Assign the selected mentor to a mentee.
            ...
            if ((index >=0) && (index<3)) {
            	...
            	cout << "Assigned ";
            	currentMentor->getName();
            	cout << " to " << mentees[index].getName() << endl;
			}
			else {
        ...
        case 6: // Task 6(g): Unassign the mentor from a mentee.
            ...
            cin >> index;
            if ((index >= 0) && (index < 3)) {
            	mentees[index].unassignMentor();
            	cout << "Mentor unassigned from " << mentees[index].getName() << endl;
			}
			else {
				cout << "Invalid index" << endl;
			}
